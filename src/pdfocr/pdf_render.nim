...
proc encodePageBitmap*(bitmap: PdfBitmap; quality: float32): seq[byte] =
  ...
  result = compressBgr(width, height, pixels = bitmap.buffer(),
      stride = bitmap.stride(), quality)

proc renderPageToWebp*(doc: PdfDocument; pageNumber: int; cfg: RenderConfig): seq[byte] =
